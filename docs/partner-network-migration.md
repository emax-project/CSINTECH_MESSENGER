...
- [x] **macOS 코드사인/공증** — 결정: **당분간 미적용, `xattr -cr`로 우회** (2026-09-22 결정).
      Gatekeeper가 "손상됨"으로 막는 건 무서명 빌드라 발생하는 정상적인 현상 — 설치 후
      `xattr -cr /Applications/CSIN-Tech.app` 한 번 실행하면 정상 실행됨. Apple Developer
      Program 계정(연 $99)이 생기면 CI에 서명·공증 단계를 추가하는 걸로 재검토.
...
3. ~~macOS 코드사인 방향 결정~~ ✅ (당분간 `xattr -cr` 우회, 2026-09-22)
4. **지금** — §5의 로그인 실패 원인 규명(최우선, 이거 없으면 나머지 검증이 불가능) →
   §5 나머지 항목 현장 검증 → §3 사내 PC 접속·구 exe 폐기 확인
...
**추가 결정 (2026-09-22, 같은 날 늦게):** macOS 무서명 건은 **당분간 `xattr -cr` 우회로 가기로
확정** — 코드사인·공증(Apple Developer 계정 필요)은 보류. 두 블로커 중 하나가 해소돼서, 이제
①(클라이언트 로그인 실패)만 §5 검증의 유일한 선결 과제로 남음.
