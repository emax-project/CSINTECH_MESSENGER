...
### 2-2. 대화방 목록 — ✅ 개선됨 (가상 스크롤) / 조직도는 이미 지연 로딩으로 해결(1-3)
...
**참고:**
- 조직도(`Main.tsx`의 `orgTree` 트리)는 이미 1-3/1-6에서 지연 로딩으로 처리돼 있어(펼친 부서만
  로드) 렌더링 자체도 대부분의 경우 이미 가벼움 — 별도 가상 스크롤 적용 안 함.
- 폴더 안 방 목록의 좌측 들여쓰기가 기존보다 약 4px 덜할 수 있음(평탄화 과정에서 부정확하게
  추정한 값을 넣느니 원래 클래스를 그대로 두는 쪽을 택함) — 육안으로 거의 안 보이는 수준.
...
| 클라이언트 | 대화방 목록 | ✅ 개선됨 (react-virtuoso 가상 스크롤) | - |
| 클라이언트 | 채팅 (DOM 누적) | 무한 스크롤 도입으로 긴 방일수록 DOM 누적 (2-1 참고) — 아직 미해결 | 데이터 많을 때 |
